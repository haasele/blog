	...
	function initFilterButtons() {
		...
		filterTags.forEach((tag) => {
			const clickHandler = function (this: HTMLElement) {
				...
				filterTags.forEach((t) => t.classList.remove("movie-active"));
				...
				const movieItems = Array.from(listContainer!.children).filter(
					(item) => item.hasAttribute("data-movie-status"),
				);
				...
				movieItems.forEach((item) => {
					const itemStatus = item.getAttribute("data-movie-status");
					const shouldShow =
			...
			window.movieFilterEventListeners.push([tag, "click", clickHandler]);
		});
...
declare global {
	interface Window {
		movieFilterEventListeners: [Element, string, () => void][];
	}
}
